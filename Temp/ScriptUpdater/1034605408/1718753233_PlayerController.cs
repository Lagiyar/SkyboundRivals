...
public class PlayerController : MonoBehaviour
{
    ...
    void HandleMovement()
    {
        // Only allow movement while grounded
        if (isGrounded)
        {
            float inputX = Input.GetAxisRaw("Horizontal");
            float inputZ = Input.GetAxisRaw("Vertical");
    ...
    {
        // No grounded movement — movement only while airborne
        if (inJump && lockedMoveDirection != Vector3.zero)
        {
            Vector3 velocity = rb.linearVelocity;
            Vector3 horizontal = lockedMoveDirection * velocity.magnitude;
